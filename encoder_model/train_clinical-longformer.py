...
def decode_labels(encoded_labels, labels = ['Dementia', 'Mild Cognitive Impairment', 'Normal Cognition']):
...
def train(args):
    ...
    # Evaluate the model
    eval_pred = trainer.evaluate()

    eval_result_save_path = os.path.join(args.output_dir, "fine-tuned", args.input, "eval_pred.json")

    if not os.path.exists(eval_result_save_path):
         dir = os.path.join(args.output_dir, "fine-tuned", args.input)
         os.makedirs(dir)
    ...
    print(f"==================== model saved in {os.path.join(args.output_dir, 'fine-tuned', args.input, 'model')} ====================")
